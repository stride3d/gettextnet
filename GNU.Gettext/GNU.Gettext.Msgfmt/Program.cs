        ...
        while ((option = getopt.getopt()) != -1)
        {
            switch (option)
            {
                ...
                case 4:
                    ...
                    break;
                case ':':
                    message.Append($"Option {getopt.OptoptStr} requires an argument");
                    return false;
                case '?':
                    message.Append($"Invalid option '{getopt.OptoptStr}'");
                    return false;
                case 'r':
        ...
        try
        {
            ...
            if (accepted)
            {
                foreach (string fileName in options.InputFiles)
                {
                    ...
                    {
                        message.Append($"File {fileName} not found");
                        accepted = false;
                    }
                }
            }
            ...
            if (accepted && options.Mode == Mode.SateliteAssembly)
            {
                ...
                {
                    message.Append($"Cannot create culture from {options.LocaleStr}");
                    accepted = false;
                }
            }
        }
