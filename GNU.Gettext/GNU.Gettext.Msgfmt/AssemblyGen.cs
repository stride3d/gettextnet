...
public class AssemblyGen
{
    ...
    private void ValidateFormatString(string s)
    {
        ...
        if (!result.Result)
        {
            throw new FormatException($"Invalid sting format: '{s}'\n{result.ErrorMessage}");
        }
    }
    ...
    private void SaveToFile()
    {
        ...
        if (!Directory.Exists(AssemblyOutDir))
            throw new Exception($"Error creating output directory {AssemblyOutDir}");
    }
    ...
    static string ToConstStr(string s) => $"@\"{s.Replace("\"", "\"\"")}\"";
